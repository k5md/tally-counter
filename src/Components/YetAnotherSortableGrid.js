...
let ListItem = React.createClass({
  ...
  render() {
    const { children, style, onMouseDown, onLayout, onTouchStart } = this.props;

    var css = ListItem.css;

    return (
      <View style={[css.container, style]} onLayout={onLayout} onTouchStart={onTouchStart}>
        {children}
      </View>
    );
  },
      ...
      return (
        <Spring key={key} endValue={style}>
          {({ position, top, width, scale, backgroundColor }) => {
            ...
            return (
              <ListItem key={key} onLayout={this.handleItemLayout.bind(this, key)} style={ss}>
                {dataRenderer(item)}
              </ListItem>
            );
          }}
    ...
    return (
      <View style={css.container}>
        ...
        */}
        ...
        >
          <View
            style={[css.list, { height: contentHeight }]}
            ref="list"
...
const App = React.createClass({
  ...
  render() {
    ...
    return (
      <View style={css.container}>
        <List items={items}>{item => <Text style={css.quoteText}>{item}</Text>}</List>
